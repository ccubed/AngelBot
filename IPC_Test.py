# This is equivalent to the other AngelBot.py files but it immediately sends an IPC message. I'm trying to track down
# an annoying IPC issue where only one process sends ipc messages per the timer.
import asyncio
import importlib
import inspect
import json
...
class AngelBot(discord.Client):
    def __init__(self, shard, shards, conn):
        ...
        self.cid = 0
        self.ipc = conn
        self.ipc.send("STATUS:{}:0:0".format(shard))
