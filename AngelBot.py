...
class AngelBot(discord.Client):
    ...
    async def on_message(self, message):
        if message.author == self.user:
            ...
            if message.server == None or message.channel == self.message_channel:
            ...
            if message.channel == self.message_channel:
                await self.send_message(message.channel,
                                        self.planner.create('raid', message.content[6:].lower(), message.server))
    ...
    async def on_ready(self):
        self.message_channel = discord.utils.get(self.get_all_channels(), name='bot-chat')
